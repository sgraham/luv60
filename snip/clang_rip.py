...
def process_obj_files_to_patch_func(
    ...
    def gen_snippet(fallthrough):
        ft = "_fallthrough" if fallthrough else "_fixup"
        arg = args_str_no_continuations if fallthrough else args_str
        if arg:
            arg = f", {arg}"
        sf.write(
            f"static inline void snip_{snip_name}{ft}(int num_int_regs_in_use, unsigned char** inout_p{arg}) {{\n"
        )
            ...
            while i < len(all_bytes):
                ...
                if r:
                    if r[0] == "IMAGE_REL_AMD64_ADDR64":
                        ...
                        if not fallthrough and r[1].startswith('$CONT'):
